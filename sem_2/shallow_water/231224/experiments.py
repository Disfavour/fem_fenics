...
import numpy as np
from shallow_water_1d import sw_1d
...
if __name__ == '__main__':
    ...
    for ms in mesh_sizes:
        for tau in taus:
            for s in sigmas:
                try:
                    t, m, E, me, Ee, components, components_exact = sw_1d(ms, tau, s)
                except Exception:
                    continue
                plot_components(t, components, components_exact, join(images_dir, f'ms{ms}_tau{tau}_s{s}_c'))
                plot_m(t, m, me, join(images_dir, f'ms{ms}_tau{tau}_s{s}_m.png'))
                plot_E(t, E, Ee, join(images_dir, f'ms{ms}_tau{tau}_s{s}_E.png'))
