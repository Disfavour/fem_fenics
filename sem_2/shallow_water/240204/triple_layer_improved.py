...
def get_solution(hl, hr, T, mesh_size, tau, theta, sigma, ts2store, info=False):
    domain_size = 5
    degree = 1
    ...
    w, wn, wnm1 = Function(W), Function(W), Function(W)
    ht, ut = TestFunctions(W)
    h, u = split(w)
    hn, un = split(wn)
    hnm1, unm1 = split(wnm1)

    we = Function(We)
    he, ue = split(we)

    hs = sigma*h + (1-2*sigma)*hn + sigma*hnm1
    us = sigma*u + (1-2*sigma)*un + sigma*unm1
    
    F = (theta*(h-hn)/tau + (1-theta)*(hn-hnm1)/tau + (hs*us).dx(0)) * ht*dx \
        + (theta*(h*u-hn*un)/tau + (1-theta)*(hn*un-hnm1*unm1)/tau + (hs*us*us).dx(0) + g*hs*(hs).dx(0)) * ut*dx
...
if __name__ == '__main__':
    ...
    get_solution(10, 1, 0.5, 200, 0.005, 0.5, 1.0, [])
