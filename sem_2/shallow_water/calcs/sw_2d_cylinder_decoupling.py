...
def shallow_water_1d(K=1, s=1, tau=0.01, mesh_size=100, T=1, degree=1):
    ...
    vtkfile = File(join(paraview, 'sw_2d_cylinder_decoupling.pvd'))
    ...
    bcs = [
        DirichletBC(U.sub(0), Constant(0), CompiledSubDomain("on_boundary && (near(x[0], -xsize) || near(x[0], xsize))", xsize=domain_size)),
        DirichletBC(U.sub(1), Constant(0), CompiledSubDomain("on_boundary && (near(x[1], -ysize) || near(x[1], ysize))", ysize=domain_size)),
    ]
    ...
    w = Function(W)
    w.rename('(h, u)', 'shallow water 2d cylinder decoupling')
    ...
    # t = 0
    h.assign(project(Expression('x[0]*x[0] + x[1]*x[1] < 2.5 ? hl : hr', hl=hl, hr=hr, degree=degree), H))
