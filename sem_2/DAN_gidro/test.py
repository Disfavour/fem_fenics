...
def symmetric(tau=0.01, mesh_size=100, vtkfile=None, verbose=False):
    ...
    bcs = [DirichletBC(V.sub(0).sub(0), Expression('-x[0]', degree=degree), boundaries, 1),
           DirichletBC(V.sub(0).sub(1), Expression('-x[1]', degree=degree), boundaries, 2)]
    ...
    if verbose:
        print(f"conservation: t {t:>5.3f}\t M {m:>15.10f}\t E {E:>15.10f}\t"
              f"I ({I0s[-1]:>15.10f}, {I1s[-1]:>15.10f})\t I_0 - sum(dI) ({I0s[0] - sum(dI0s):>15.10f}, {I1s[0] - sum(dI1s):>15.10f})")
...
if __name__ == '__main__':
    ...
    print(join(data, 'sym_tau0.01_ms100.npy'))
    print(isfile(join(data, 'sym_tau0.01_ms1010.npy')))
