...
if __name__ == '__main__':
    weighted(s=1, tau=0.01, mesh_size=100, vtkfile=join(paraview, 's111ms200tsmall.pvd'), verbose=True)
    #print(join(paraview, 's1ms200tsmall.pvd'))
    ...
    # x = mesh.coordinates()
    # print(x[(x[:, 1] == 0) & (x[:, 0] >= 0)])
    # indices = np.where((x[:, 1] == 0) & (x[:, 0] >= 0))[0]
    # print(indices)
    # print(x[indices])
