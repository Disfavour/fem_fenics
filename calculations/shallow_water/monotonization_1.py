...
def calculate(hl, hr, T, mesh_size, tau, alfa, gamma, ts2store, info=False):
    ...
    # F = (dth - a*tau**2*(b*dth.dx(0)).dx(0) + (h*u).dx(0)) * ht*dx
    # F = (dth + (h*u).dx(0)) * ht*dx - a*tau**2*(b*ht*dth.dx(0)).dx(0)*dx + a*tau**2*b*dth.dx(0)*ht.dx(0)*dx
    F = (dth + (h*u).dx(0)) * ht*dx + alfa*tau**2*b*dth.dx(0)*ht.dx(0)*dx(mesh)
