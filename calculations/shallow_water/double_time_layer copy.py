...
def calculate(hl, hr, T, mesh_size, tau, degree, sigma, ts2store, info=False):
    domain_size = 5
    #degree = 1
    ...
    # F = (dt(h) + s(h*u).dx(0)) * ht*dx \
    #     + (s(h)*dt(u) + s(u)*dt(h) + s(h*u**2).dx(0) + g/2*s(h**2).dx(0)) * ut*dx
...
if __name__ == '__main__':
    ...
    ts, m, E, m_e, E_e, err_h, err_u, x, h_, u_, x_e, h_e, u_e = calculate(2, 1, 0.9, 400, 0.01, 1, 1, [0.3, 0.6, 0.9], True)
