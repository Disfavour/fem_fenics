...
def calculate(hl, hr, T, mesh_size, tau, theta, sigma, ts2store, info=False):
    ...
    w, wn, wn1 = Function(W), Function(W), Function(W)
    ...
    dt = lambda f: theta*(f[0]-f[1])/tau + (1-theta)*(f[1]-f[2])/tau
    s = lambda f: sigma*f[0] + (1-2*sigma)*f[1] + sigma*f[2]
...
if __name__ == '__main__':
    ...
    calculate(2, 1, 0.5, 400, 0.01, 0.5, 0.75, [], True)
