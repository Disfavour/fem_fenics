...
def calculate(mesh_size, tau, degree, T, ts2store, sigma, alfa, gamma, kappa, ntest, info=False):
    ...
    bc = DirichletBC(P, Constant(1), CompiledSubDomain('on_boundary && near(x[0], 0)', left=-domain_size))
    ...
    t = 0
    if ntest == 1:
        u.assign(project(Expression('x[0] < 1 ? 1 : 2', domain_size=domain_size, degree=1), P))
    # elif ntest == 2:
...
if __name__ == '__main__':
    ...
    x, u, t, m = calculate(mesh_size=400, tau=0.01, degree=2, T=2.0, ts2store=[0.0, 1.0, 2.0], sigma=0.5, alfa=10, gamma=0, kappa=0.0, ntest=1, info=True)
    plt.plot(x, u.T)
    ...
    def exact(t):
        xs = x.copy()
        xs = (xs <= x0 + a*t)*a + (xs > x0 + a*t)*(xs <= x0 + b*t)*(xs - x0)/t + (xs > x0 + b*t) * b
