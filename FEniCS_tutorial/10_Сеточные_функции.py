...
'# Значения в узлах `compute_vertex_values()` и вектор степеней свободы `vector()`'
...
'Вектор степеней свободы (degrees of freedom)'
st.code('u.vector()[:]')
st.code(u.vector()[:])
r'''
...
'Карта преобразования порядка степеней свободы в порядок узлов сетки'
st.code('dof_to_vertex_map(P)')
st.code(dof_to_vertex_map(P))

'Преобразование порядка степеней свободы в порядок узлов сетки'
st.code('u.vector()[dof_to_vertex_map(P)]')
st.code(u.vector()[dof_to_vertex_map(P)])

'Карта преобразования порядка узлов сетки в порядок степеней свободы'
st.code('vertex_to_dof_map(P)')
st.code(vertex_to_dof_map(P))

'Преобразование порядка узлов сетки в порядок степеней свободы'
...
'Порядок степеней свободы подпространств'
st.code('W.sub(0).dofmap().dofs(), W.sub(1).dofmap().dofs()')
st.code((W.sub(0).dofmap().dofs(), W.sub(1).dofmap().dofs()))

'Вектора степеней свободы подпространств'
st.code('w.vector()[W.sub(0).dofmap().dofs()], w.vector()[W.sub(1).dofmap().dofs()]')
st.code((w.vector()[W.sub(0).dofmap().dofs()], w.vector()[W.sub(1).dofmap().dofs()]))

'Карта преобразования порядка степеней свободы в порядок узлов сетки для подпространств'
st.code('dof_to_vertex_map(W)[W.sub(0).dofmap().dofs()], dof_to_vertex_map(W)[W.sub(1).dofmap().dofs()]')
st.code((dof_to_vertex_map(W)[W.sub(0).dofmap().dofs()], dof_to_vertex_map(W)[W.sub(1).dofmap().dofs()]))

'Преобразование порядка степеней свободы в порядок узлов сетки для подпространств'
st.code('w.vector()[dof_to_vertex_map(W)[W.sub(0).dofmap().dofs()]], w.vector()[dof_to_vertex_map(W)[W.sub(1).dofmap().dofs()]]')
st.code((w.vector()[dof_to_vertex_map(W)[W.sub(0).dofmap().dofs()]], w.vector()[dof_to_vertex_map(W)[W.sub(1).dofmap().dofs()]]))

'# Конечно-элементное приближение и сеточные функции'
'Новая функция по умолчанию инициализируется нулями'
with st.echo():
    q = Function(W)
