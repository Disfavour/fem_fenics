...
F2 = dti * (r1 * u - r_old * u_old) * u_t * dx() + (r1 * u * uk).dx(0) * u_t * dx() + (r1 * u * vk).dx(
    1) * u_t * dx() + gam * r1 ** (gam - 1) * r1.dx(0) * u_t * dx()
a2 = lhs(F2)
L2 = rhs(F2)

F3 = dti * (r1 * v - r_old * v_old) * v_t * dx() + (r1 * v * uk).dx(0) * v_t * dx() + (r1 * v * vk).dx(
    1) * v_t * dx() + gam * r1 ** (gam - 1) * r1.dx(1) * v_t * dx()
a3 = lhs(F3)
L3 = rhs(F3)
...
np.save("decoupled_seidel2.npy", np.array(Es))
