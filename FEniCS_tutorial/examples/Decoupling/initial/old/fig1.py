...
F = dti*(r - r_old)*r_t*dx() + (r*u).dx(0)*r_t*dx() + (r*v).dx(1)*r_t*dx() \
    + dti*(r*u - r_old*u_old)*u_t*dx() + (r*u*u).dx(0)*u_t*dx() + (r*u*v).dx(1)*u_t*dx() + gam*r**(gam-1)*r.dx(0)*u_t*dx() \
    + dti*(r*v - r_old*v_old)*v_t*dx() + (r*v*u).dx(0)*v_t*dx() + (r*v*v).dx(1)*v_t*dx() + gam*r**(gam-1)*r.dx(1)*v_t*dx()
...
while t < T - dt/2:
    ...
    solve(F == 0, w, bcs, solver_parameters={"newton_solver": 
            {"relative_tolerance": 1e-15, "absolute_tolerance": 1e-15, "relaxation_parameter": 1.0}})
    w_old.assign(w)
    u, v, r = w.split(True)
